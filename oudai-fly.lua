...
local fr=Instance.new("Frame")fr.Size=UDim2.new(0,300,0,220)fr.Position=UDim2.new(0.5,-150,0.2,0)fr.BackgroundColor3=Color3.fromRGB(25,20,45)fr.BackgroundTransparency=0.1 fr.BorderSizePixel=0 fr.Active=true fr.Parent=sg
...
local gr=Instance.new("UIGradient")gr.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(147,51,234)),ColorSequenceKeypoint.new(0.5,Color3.fromRGB(99,102,241)),ColorSequenceKeypoint.new(1,Color3.fromRGB(59,130,246))})gr.Rotation=135 gr.Transparency=NumberSequence.new({NumberSequenceKeypoint.new(0,0.9),NumberSequenceKeypoint.new(1,0.95)})gr.Parent=fr
...
local ti=Instance.new("TextLabel")ti.Size=UDim2.new(1,-20,1,0)ti.Position=UDim2.new(0,15,0,0)ti.BackgroundTransparency=1 ti.Text="oudai fly"ti.TextColor3=Color3.fromRGB(255,255,255)ti.TextSize=22 ti.Font=Enum.Font.GothamBold ti.TextXAlignment=Enum.TextXAlignment.Left ti.Parent=tb
...
local tg=Instance.new("TextButton")tg.Name="ToggleBtn"tg.Size=UDim2.new(1,0,0,60)tg.BackgroundColor3=Color3.fromRGB(147,51,234)tg.Text="ENABLE FLY"tg.TextColor3=Color3.new(1,1,1)tg.TextSize=18 tg.Font=Enum.Font.GothamBold tg.AutoButtonColor=false tg.Parent=co
...
local mb=Instance.new("TextButton")mb.Size=UDim2.new(0,50,0,40)mb.Position=UDim2.new(0.5,-95,0,30)mb.BackgroundColor3=Color3.fromRGB(99,102,241)mb.Text="-"mb.TextColor3=Color3.new(1,1,1)mb.TextSize=24 mb.Font=Enum.Font.GothamBold mb.AutoButtonColor=false mb.Parent=sc
local mbc=Instance.new("UICorner")mbc.CornerRadius=UDim.new(0,10)mbc.Parent=mb
local pb=Instance.new("TextButton")pb.Size=UDim2.new(0,50,0,40)pb.Position=UDim2.new(0.5,45,0,30)pb.BackgroundColor3=Color3.fromRGB(147,51,234)pb.Text="+"pb.TextColor3=Color3.new(1,1,1)pb.TextSize=24 pb.Font=Enum.Font.GothamBold pb.AutoButtonColor=false pb.Parent=sc
local pbc=Instance.new("UICorner")pbc.CornerRadius=UDim.new(0,10)pbc.Parent=pb
local d,di,ds,dsp=false,nil,nil,nil
tb.InputBegan:Connect(function(i)if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then d=true ds=i.Position dsp=fr.Position i.Changed:Connect(function()if i.UserInputState==Enum.UserInputState.End then d=false end end)end end)
...
local function a(b,n,h)b.InputBegan:Connect(function(i)if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then T:Create(b,TweenInfo.new(0.15),{BackgroundColor3=h}):Play()end end)b.InputEnded:Connect(function(i)if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then T:Create(b,TweenInfo.new(0.15),{BackgroundColor3=n}):Play()end end)end
...
local bv,bg
local function sf()f=true g()h.PlatformStand=true bv=Instance.new("BodyVelocity")bv.MaxForce=Vector3.new(600000,900000,600000)bv.Velocity=Vector3.new(0,0,0)bv.Parent=rp bg=Instance.new("BodyGyro")bg.MaxTorque=Vector3.new(400000,400000,400000)bg.P=15000 bg.Parent=rp tg.Text="DISABLE FLY"tg.BackgroundColor3=Color3.fromRGB(239,68,68)sd.BackgroundColor3=Color3.fromRGB(255,70,70)T:Create(sd,TweenInfo.new(0.5,Enum.EasingStyle.Quad,Enum.EasingDirection.InOut,-1,true),{Size=UDim2.new(0,16,0,16),Position=UDim2.new(1,-32,0.5,-8)}):Play()end
local function st()f=false if bv then bv:Destroy()end if bg then bg:Destroy()end if h then h.PlatformStand=false end tg.Text="ENABLE FLY"tg.BackgroundColor3=Color3.fromRGB(147,51,234)sd.BackgroundColor3=Color3.fromRGB(0,255,120)T:Create(sd,TweenInfo.new(0.2),{Size=UDim2.new(0,12,0,12),Position=UDim2.new(1,-30,0.5,-6)}):Play()end
R.Heartbeat:Connect(function()if not f or not rp or not bv then return end local m=h.MoveDirection local v=m.Z*-30 if U:IsKeyDown(Enum.KeyCode.Space)then v=v+60 end if U:IsKeyDown(Enum.KeyCode.LeftControl)then v=v-60 end bv.Velocity=(m*s)+Vector3.new(0,v,0)bg.CFrame=workspace.CurrentCamera.CFrame end)
