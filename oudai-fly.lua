...
local bv,bg
local function sf()f=true g()rp.Anchored=false h.PlatformStand=true bv=Instance.new("BodyVelocity")bv.MaxForce=Vector3.new(math.huge,math.huge,math.huge)bv.Velocity=Vector3.new(0,0,0)bv.Parent=rp bg=Instance.new("BodyGyro")bg.MaxTorque=Vector3.new(math.huge,math.huge,math.huge)bg.CFrame=rp.CFrame bg.Parent=rp tg.Text="DISABLE FLY"tg.BackgroundColor3=Color3.fromRGB(220,38,38)sd.BackgroundColor3=Color3.fromRGB(255,100,100)end
local function st()f=false if bv then bv:Destroy()end if bg then bg:Destroy()end if h then h.PlatformStand=false end if rp then rp.Anchored=false end tg.Text="ENABLE FLY"tg.BackgroundColor3=Color3.fromRGB(147,51,234)sd.BackgroundColor3=Color3.fromRGB(0,255,120)end
