...
R.Heartbeat:Connect(function()if not f or not rp or not bv then return end local fx,fz=0,0 if U:IsKeyDown(Enum.KeyCode.W)then fz=fz-1 end if U:IsKeyDown(Enum.KeyCode.S)then fz=fz+1 end if U:IsKeyDown(Enum.KeyCode.A)then fx=fx-1 end if U:IsKeyDown(Enum.KeyCode.D)then fx=fx+1 end local fy=0 if U:IsKeyDown(Enum.KeyCode.Space)then fy=fy+1 end if U:IsKeyDown(Enum.KeyCode.LeftControl)then fy=fy-1 end local cam=workspace.CurrentCamera bv.Velocity=((cam.CFrame.RightVector*fx+cam.CFrame.LookVector*fz)*Vector3.new(1,0,1).Unit+Vector3.new(0,1,0)*fy)*s bg.CFrame=rp.CFrame end)
